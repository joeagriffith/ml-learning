...
class Feedforward(nn.Module):
    def __init__(self, sizes, activation=nn.ReLU(), bias=True, momentum=0.95, dropout=0.0):
        super().__init__()
        ...
        self.target_actvs = [None for _ in self.layers]
    
    def update_target_actvs(self, x, layer_i):
        if self.target_actvs[layer_i] is None:
            self.target_actvs[layer_i] = x.mean().item()
        else:
