...
def generate(
        ...
        input_image=None, 
        strength=0.8, # How much noise to add to input_image. (less strength means output is more similar to input_image)
        do_cfg=True, 
        cfg_scale=7.5, # How much to listen to prompt
        sampler_name="ddpm", 
    ...
    with torch.no_grad():
        ...
        assert strength > 0.0 and strength <= 1.0, "Strength must be > 0 and <= 1"
        ...
        if do_cfg:
            ...
            # (Batch_Size(1), Seq_Len) -> (Batch_Size(1), Seq_Len, Dim)
            ...
            # 2 x (Batch_size(1), Seq_Len, Dim) -> (2xBatch_Size, Seq_Len, Dim) = (2, 77, 768)
        ...
        if input_image: 
            ...
            input_image_tensor = input_image_tensor.unsqueeze(0).permute(0, 3, 1, 2).contiguous()
        ...
        for i, timestep in enumerate(timesteps):
            ...
            if do_cfg:
                ...
                # repeat the input, one will be used with the prompt, and one without
